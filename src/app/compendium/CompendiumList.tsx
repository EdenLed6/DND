...
function DetailBody({ tab, d }: { tab: string; d: any }) {
  ...
  if (tab === "spells") return (
    <div>
      ...
      <p className="text-sm text-[#a9977c]">{d.level === 0 ? "Cantrip" : `Level ${d.level}`} · {d.school}
      ...
      {d.higherLevel && <p className="mt-2 text-sm text-[#c9bda5]"><b>At Higher Levels:</b> {d.higherLevel}</p>}
    </div>
  );
  ...
  if (tab === "monsters") return (
    <div>
      <h2 className="font-display text-2xl text-gold">{d.name}</h2>
      <p className="text-sm text-[#a9977c]">{d.size} {d.type}{d.subtype ? ` (${d.subtype})` : ""} · {d.alignment} · CR {d.cr} ({d.xp} XP)</p>
      ...
      <div className="mt-2 grid grid-cols-6 gap-1 text-center text-sm">
        {["str","dex","con","int","wis","cha"].map((a) => (
          <div key={a} className="stat-box"><div className="text-[10px] uppercase text-[#a9977c]">{a}</div><div>{d[a]}</div></div>
        ))}
      </div>
  ...
  if (tab === "equipment") return (
    <div>
      <h2 className="font-display text-2xl text-gold">{d.name}</h2>
      <p className="text-sm text-[#a9977c]">{d.category}{d.costGp ? ` · ${d.costGp} ${d.costUnit}` : ""}{d.weight ? ` · ${d.weight} lb` : ""}</p>
  ...
  if (tab === "magic") return (
    <div>
      <h2 className="font-display text-2xl text-gold">{d.name}</h2>
      <p className="text-sm text-[#a9977c]">{d.rarity} · {d.type}{d.requiresAttunement ? " · requires attunement" : ""}</p>
  ...
  if (tab === "races") return (
    <div>
      <h2 className="font-display text-2xl text-gold">{d.name}</h2>
      <p className="text-sm text-[#a9977c]">{d.size} · speed {d.speed} · {J(d.abilityBonuses).map((a: any) => `${a.ability} +${a.bonus}`).join(", ")}</p>
      <StatList title="Traits" arr={J(d.traits)} />
      {d.languages && <p className="text-sm"><b>Languages:</b> {d.languages}</p>}
    </div>
  );
  ...
  if (tab === "classes") return (
    <div>
      <h2 className="font-display text-2xl text-gold">{d.name}</h2>
      <p className="text-sm text-[#a9977c]">Hit die d{d.hitDie} · Saves {J(d.savingThrows).join(", ")}{d.spellcastingAbility ? ` · Caster (${d.spellcastingAbility})` : ""}</p>
